...
class QueryBuilderHelper
{
    ...
    public function processStandardFilter(QueryBuilder $qb, $name, $fields, $values) :QueryBuilder
    {
        ...
        if (is_array($fields)) {
            ...
            foreach ($fields AS $field) {
                $orx->add(new Comparison($field, $op, ':' . $name));
            }
        ...
        $qb->andWhere(new Comparison($fields, $op, ':'.$name));
        $qb->setParameter($name, $values);
    ...
    public function processArrayValueFilter(QueryBuilder $qb, $name, $fields, $values) :QueryBuilder
    {
        ...
        $qb->andWhere($qb->expr()->in($fields, ':' . $name));
        $qb->setParameter($name, $values);
        ...
        if (0 === count($sort)) {
            foreach ($defaults AS $k => $v) {
                if (strtoupper($v) === 'DESC') {
                    $qb->addOrderBy($qb->expr()->desc($k));
                    continue;
                }
                $qb->addOrderBy($qb->expr()->asc($k));
            }
